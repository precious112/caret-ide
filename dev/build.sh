...
# Caret branding (overridable via environment, e.g. in CI).
export APP_NAME="${APP_NAME:-Caret}"
export ASSETS_REPOSITORY="${ASSETS_REPOSITORY:-precious112/caret-ide}"
export BINARY_NAME="${BINARY_NAME:-caret}"
export CI_BUILD="${CI_BUILD:-no}"
export GH_REPO_PATH="${GH_REPO_PATH:-precious112/caret-ide}"
export ORG_NAME="${ORG_NAME:-caretAI}"
# Manual/direct distribution for now: no update server.
...
if [[ "${SKIP_BUILD}" == "no" ]]; then
  ...
  # Bundle the Caret extension as a built-in into the freshly-built app.
  if [[ "${SKIP_CARET_EXTENSION}" != "yes" ]]; then
    . ./build/caret/add-builtin-extension.sh
  fi
